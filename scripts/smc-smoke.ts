...
assert.ok(riskPctFromConfidence(60) <= 2.5);
...
assert.equal(risk.riskPct, 4);
assert.ok(risk.riskEur >= 30, `riskEur ${risk.riskEur}`);
assert.ok(risk.riskEur <= 45, `riskEur trop haut ${risk.riskEur}`);
assert.ok(risk.notionalEur > 1500, `notional trop petit: ${risk.notionalEur}`);
...
if (setup.checklist.allPass && setup.risk) {
  assert.ok(
    setup.risk.riskPct >= 1.5 && setup.risk.riskPct <= 4,
    `setup sûr risque hors bande 1.5–4%, got ${setup.risk.riskPct}`,
  );
}
